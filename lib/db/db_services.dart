...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> geContact_ModelMapList() async {
  ...
  Future<int> inserContact_Model(Contact_Model contact) async {
    Database db = await this.database;
  ...
  Future<int> updateContact(Contact_Model contact) async {
    Database db = await this.database;
  ...
  Future<List<Contact_Model>> geContact_ModelList() async {
    var contactMapList =
    await geContact_ModelMapList(); // Get 'Map List' from database
    int count =
        contactMapList.length; // Count the number of map entries in db table

    List<Contact_Model> contactList = <Contact_Model>[];
    // For loop to create a 'Contact List' from a 'Map List'
    for (int i = 0; i < count; i++) {
      contactList.add(Contact_Model.fromMapObject(contactMapList[i]));
    }
